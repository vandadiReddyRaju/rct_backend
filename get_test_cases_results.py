...
def get_question_details(question_command_id, column_name):
    ...
    try:
        ...
        if result.empty:
            return f"Question ID '{question_command_id}' not found in the CSV."
...
def get_test_case_results(question_command_id):

    command = (
        f'cd {get_question_details(question_command_id,"question_folder_location")} && '
        'rm -rf node_modules && '
        'pnpm i -D --save-exact jest-watch-typeahead@0.6.5 && '
        f'cd {get_question_details(question_command_id,"question_tmp_folder_location")}  && cp -rf __tests__ {get_question_details(question_command_id,"question_folder_location")}/src  && cd {get_question_details(question_command_id,"question_folder_location")} && npm test '
    )
...
#     if len(sys.argv) != 2:
#         print("Usage: python script.py <question_command_id>")
#         sys.exit(1)
    
#     question_command_id = sys.argv[1]
#     results = get_test_case_results(question_command_id)
    
#     # Save results to a JSON file
#     with open(f'test_results_{question_command_id}.json', 'w') as f:
#         json.dump(results, f)